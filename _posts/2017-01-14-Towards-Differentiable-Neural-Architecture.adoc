...
Lets look at this step by step. Note that this implementation only illustrates shared weights for all features of `layer_inputs[i]` for simplicity. Se e the full implementation on link:https://github.com/raghakot/deep-learning-experiments/blob/master/exp3/architecture.py#L8[github] for creating a unique weight per feature map from previous layers.

...
To get a clearer picture, lets compute the box plot by considering 1000 random test images as described earlier. This makes it pretty clear that cannot just add rotation operations to a _regular_ model. On the flip side, it provides evidence that rotational conv affects the weights and optimization process.
...
* Even though convergence is slower, 8 rotations seem to offer better invariance comapred to 4 rotations.
* It is interesting to note that all models exhibit the same seasonal pattern of accuracy with respect to rotation (see fig. 9). What's with the weird dip at \(100^{\circ}\)? 
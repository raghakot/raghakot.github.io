...
* Output the volume \(V_{i} \mid \arg\max_{i} \sum ReLU(V_{i})\). i.e., choose the activation volume that has the maximum sum after `ReLU`. `ReLU` is since we want to consider the volume that has the most `positive` activations, which indicates similarity.

...
[.text-center]
.Training convergence graphs for loss and accuracy on validation set (blue = old model, red = new model)
...
First thought might be to use a `linear` activation function, but this limits the representational power. A quick experiment with `ELU` activation confirms this intuition by reaching `val_loss=` and `val_accuracy=`.
